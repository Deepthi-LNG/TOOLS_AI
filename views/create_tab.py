...
def render_create_tab():
    """Render the Create New tab content with dark theme"""
    # Header instruction
    st.markdown("""
    <p style="margin-bottom: 15px; font-size: 0.95rem; opacity: 0.8;">
        Enter your task details below. Be specific about your project goals and requirements.
    </p>
        ...
        if st.button("🔮 Generate To-Do List", use_container_width=True):
            if task_description.strip():
                with st.spinner("Generating your professional to-do list..."):
                    to_do_list = generate_to_do_list(task_description, extra_instructions)
